...
CONTAINS
   ...
   SUBROUTINE inferences( kt, Kbb, Kmm, Kaa )
      ...
      ! Field 1e
      infsnd(jps_1)%z3(:,:,1:ssnd(ntypinf,jps_1)%nlvl) = ts(:,:,1:ssnd(ntypinf,jps_1)%nlvl,jp_tem,Kmm)
      !
      ! Field 2
      infsnd(jps_2)%z3(:,:,1:ssnd(ntypinf,jps_2)%nlvl) = ts(:,:,1:ssnd(ntypinf,jps_2)%nlvl,jp_sal,Kmm)
      !
      ! ========================
